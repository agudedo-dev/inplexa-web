...
(()=>{
  ...
  const startHero=()=>{stopHero();if(!reduced.matches&&!document.hidden)heroTimer=setInterval(()=>showHero(heroIndex+1),5600);};
  ...
  reduced.addEventListener('change',e=>{if(e.matches){paused=true;sync();stop();stopHero();}else{paused=false;sync();start();startHero();}});
  sync();start();

  const progress=document.querySelector('.scroll-progress i');
  const parallaxItems=[...document.querySelectorAll('.hero-shots,.process-visual>img,.factory-slide img')];
  let ticking=false;
  const updateScroll=()=>{const max=Math.max(document.documentElement.scrollHeight-innerHeight,1);const ratio=Math.min(scrollY/max,1);progress.style.transform=`scaleX(${ratio})`;if(!reduced.matches){parallaxItems.forEach(item=>{const rect=item.closest('.hero-visual,.process-visual,.factory-slider').getBoundingClientRect();if(rect.bottom>0&&rect.top<innerHeight){const offset=(rect.top+rect.height/2-innerHeight/2)*-.035;item.style.setProperty('--parallax',`${offset.toFixed(1)}px`);}});}ticking=false;};
